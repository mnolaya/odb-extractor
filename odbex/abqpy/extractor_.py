    ...
    for i, frame in enumerate(frames):
        ...
        for rid, fdd in field_data_dicts.items():
            ...
            for field_name, field_dict in fdd.items():
                ...
                # Temporary implementation for area-weighted averaged traction-separation behavior of axisymmetric model
                if field_name in ['CSHEAR1', 'CSLIP1'] and region.nodes is not None and type(region.nodes) == tuple:
                    r_pos = [node.coordinates[0] for nodes in region.nodes for node in nodes]
                    weights = []
                    for r0, r1 in zip(r_pos[:-1], r_pos[1:]):
                        dr = abs(r1 - r0)
                        rmid = 0.5*(r0 + r1)
                        weights.append(2*np.pi*rmid*dr)
                    weights = np.array(weights)
                    fd_ = []
                    for fd0, fd1, w in zip(fd[:-1, 0], fd[1:, 0], weights):
                        fd_.append(0.5*(fd0 + fd1))
                    fd_mean = np.array([np.sum(fd_*weights)/np.sum(weights)])
                    fd_std = np.std([fd_*weights])
                else:
